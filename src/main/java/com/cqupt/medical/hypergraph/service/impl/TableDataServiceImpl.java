...
import com.cqupt.medical.hypergraph.mapper.TableDataMapper;
...
public class TableDataServiceImpl implements TableDataService {
    ...
    @Override
    public List<String> uploadFile(MultipartFile file, String tableName, String type, String user, String userId, String parentId, String parentType,String status,Double size,String is_upload,String is_filter) throws IOException, ParseException, CsvException {
...
//        }
        // 创建表头信息 获取宽表字段管理信息
        List<FieldEntity> fields = fieldManageService.list(null);
...
//        }
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public List<String> storeTableData(MultipartFile file, String tableName) throws IOException, CsvException {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public List<String> ParseFileCol(MultipartFile file, String tableName) throws IOException, CsvException {
        ArrayList<String> featureList = null;
        if (!file.isEmpty()) {
            // 使用 OpenCSV 解析 CSV 文件
            Reader reader = new BufferedReader(new InputStreamReader(file.getInputStream(),"UTF-8"));
